...
import mix, { toUnicode, supportedEmojis } from "emoji-mixer";
...
const searchObjectByValue = (obj, value) => {
    for (let key in obj)
...
const extractEmojis = string => {
    ...
    for (let i = 0; i < content.length; i++) {
        ...
        const unicode = toUnicode(content[i], true);
        if (supportedEmojis.includes(unicode))
            result.push(unicode);
    }
...
export default {
    ...
    async run(client, userDb, message) {
        ...
        const url = mix(emojis[0], emojis[1], false, true);

        if (!url)
            return message.reply({ content:
                client.constants.getRandomResponse(badExperimentResponses)
            })

        downloadImage(url).then(buffer => {
            message.channel.send({ files: [
                new discord.AttachmentBuilder( 
                    buffer, { name: "Experiment.png" }
                )
            ] })
        }).catch(error => {
            console.error(error)
