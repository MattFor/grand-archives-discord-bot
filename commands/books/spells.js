...
export default {
    ...
    async run(bot, userDb, message) {
        ...
        let clientSpells = organizeCollectables(Object.keys(bot.collectables)
        ...
        for (const rarity of Object.keys(clientSpells).map(n => { return Number(n) }).sort((a, b) => b - a)) {
            ...
            for (const title of Object.keys(clientSpells[rarity])) {
                if (JSON.stringify(clientSpells[rarity][title]).includes("undefined")) {
                    ...
                    for (const [key, value] of Object.entries(bot.collectables))
                        if (value.title === title) {
                            collectable = value
                            break
                        }
                        description += `[${title}](${
                            bot.channels.cache.get(collectable.channel).url
                        })\n`
                    continue
                }
                ...
                for (const chapter of Object.keys(clientSpells[rarity][title])) {
                    ...
                    const pages = `${clientSpells[rarity][title][chapter].map(page => {
                        ...
                        const channel = bot.channels.cache.get(article.channel)
